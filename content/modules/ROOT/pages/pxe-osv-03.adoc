= Live migrations and high availability
...
image:livemigrate-vm-01.png[get ocp node,link=self,window=blank]

We can now migrate the VM to a new node by selecting the `Actions` menu,
and clicking `Migrate` and leaving `Automatically Select Node`

image:livemigrate-vm-02.png[migrate vm,link=self,window=blank]
...
[source,sh,role=execute]
----
NODENAME=$(oc get pods -o wide -n vmtest | grep stream9-example| grep 'Running' | awk '{print $7}' | head -n 1)
...
====
Let's watch our VM and pod status in the vmtest namespace:

[source,sh,role=execute]
----
watch oc get pods,vms -o wide -n vmtest
----
