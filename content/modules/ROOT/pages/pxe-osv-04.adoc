...
In this scenario, we will create a virtual machine snapshot.
...
image:snapshot-vm-01.png[Select VM,link=self,window=blank]

Navigate to the `Snapshots` tab and select `Take Snapshot`

image:snapshot-vm-02.png[Take Snapshot,link=self,window=blank]
...
image:snapshot-vm-03.png[Save Snapshot,link=self,window=blank]
...
image:snapshot-vm-04.png[Stop VM,link=self,window=blank]
...
image:snapshot-vm-05.png[restore VM,link=self,window=blank]

====
[NOTE]
Restoring a snapshot is a destructive operation as it
discards all changes that were made to a virtual machine since the
snapshot was taken. To avoid losing data, it is possible to take a
...
image:snapshot-vm-06.png[start VM,link=self,window=blank]
