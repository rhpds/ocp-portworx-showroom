= Automatic storage management with Autopilot
...
If you would like a seperate window clicking {openshift_cluster_console_url}[OpenShift Web Console^] open the OpenShift Console in a new tab window.
...
oc label pvc centos-stream9-autopilot-data-disk \
  expand=true --overwrite -n vmtest
----

[source,sh,role=execute]
----
oc get pvc centos-stream9-autopilot-data-disk -n vmtest
----
...
until virtctl ssh cloud-user@vm/centos-stream9-autopilot/vmtest -i ~/.ssh/id_rsa -t "-o StrictHostKeyChecking=no" -c 'lsblk'; do
    echo "waiting for VM to boot"
    sleep 10
done

# Set up the filesystem and mount the disk as /data
virtctl ssh cloud-user@vm/centos-stream9-autopilot/vmtest \
  -i ~/.ssh/id_rsa \
...
=== Task 5: Fill the disk

We will use the `shred` command to fill up storage on our virtual machine to trigger Autopilot.
...
=== Task 6: Observe the Portworx Autopilot events
...
=== Task 7: Verify the volume expansion
...
[source,sh,role=execute]
----
virtctl ssh cloud-user@vm/centos-stream9-autopilot/vmtest -i ~/.ssh/id_rsa -t "-o StrictHostKeyChecking=no" -c 'df -h'
----
...
Congratulations on completing the Portworx Enterprise with OpenShift Virtualization lab!

Throughout this lab you have:

* Installed and configured **Portworx Enterprise** on an OpenShift cluster
* Provisioned **virtual machines** backed by Portworx persistent storage
* Performed **live migrations** and validated high availability across node failures
* Used **Portworx snapshots** to protect and restore a virtual machine
* Leveraged **Portworx Autopilot** to automatically expand storage capacity in response to real demand — with zero downtime and no manual intervention

Portworx Enterprise provides a powerful, enterprise-grade storage foundation for OpenShift Virtualization workloads, ensuring your virtual machines are resilient, portable, and efficiently managed at scale.

We hope you found this lab valuable. For further reading, visit the https://docs.portworx.com[Portworx documentation^] and the https://docs.openshift.com/container-platform/latest/virt/about_virt/about-virt.html[OpenShift Virtualization documentation^].
